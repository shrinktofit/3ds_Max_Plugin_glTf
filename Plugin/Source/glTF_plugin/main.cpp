...
using class_descriptions_t =
    std::tuple<plugin::glTF_exporter::class_description>;
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL,
  ...
  if (fdwReason == DLL_PROCESS_ATTACH) {
    ...
    plugin::win32::set_instance(hinstDLL);
    DebugPrint(_T("glTf plugin loaded."));
    DisableThreadLibraryCalls(plugin::win32::get_instance());
  }
  return (TRUE);
}
...
__declspec(dllexport) const TCHAR *LibDescription() {
  return plugin::win32::get_string_resource(IDS_PLUGIN_DESCRIPTION);
}
